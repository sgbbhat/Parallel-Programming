...
2) Edit the source files matrixmul.cu and matrixmul_kernel.cu to complete 
   ...
   mp3-tiled-matrixmul in your SDK C/bin/linux/release directory. 
...
3) There are several modes of operation for the application. Note that the file
   ...
   N.width, respectively . The second and third arguments will be expected to
...
4) Submit your solution via Moodle as a tarball. Your submission should
   contain the mp3-tiled-matrixmul folder provided, with all the changes and
   ...
   It may be helpful to search for a document called "The CUDA Compiler Driver NVCC" 
   and read up on "Printing code generation statistics". Information on the compute 
   capabilities for various GPUs can also be found online (e.g. http://en.wikipedia.org/wiki/CUDA).
...
Functionality/knowledge: 65%
    ...
    - Handles matices of arbitrary sizes. 
