...
float computeOnDevice(float* h_data, int num_elements)
{
	...
	numOutputElements = num_elements / (BLOCK_SIZE<<1);
	if (num_elements % (BLOCK_SIZE<<1)) {
		numOutputElements++;
	}

hostOutput = (float*) malloc(numOutputElements * sizeof(float));
cudaMalloc(&deviceInput, sizeof(float) * num_elements);
cudaMalloc(&deviceOutput, sizeof(float) * numOutputElements);

cudaMemcpy(deviceInput, h_data, sizeof(float) * num_elements, cudaMemcpyHostToDevice);

dim3 dimGrid(numOutputElements, 1, 1);
dim3 dimBlock(BLOCK_SIZE, 1, 1);

reduction<<<dimGrid, dimBlock>>>(deviceInput, deviceOutput, num_elements);
